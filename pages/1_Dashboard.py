...
def plot_ativo(df, ticker, nome_empresa, vcp_detectado=False):
   ...
   # Médias móveis (primeiro, para ficarem atrás)
    ...
    caminho_img = f"/tmp/{ticker}.jpg"
    try:
        fig.write_image(caminho_img, format="jpg", width=1600, height=900)
    except Exception as e:
        print(f"Erro ao salvar imagem para {ticker}: {e}")
        caminho_img = None
    ...
    for i, ticker in enumerate(tickers):
        ...
        try:
            ...
            with st.container():
                ...
                if st.button(f"⭐ Adicionar {ticker} aos favoritos", key=f"fav_{ticker}"):
                    try:
                        uid = st.session_state.user["localId"]
                        ...
                        st.success(f"✅ {ticker} salvo como favorito.")
                    except Exception as e:
                        ...
                        if os.path.exists(caminho_img):
                            with open(caminho_img, "rb") as img_file:
                                st.download_button(
                                    ...
                                    data=img_file,
                                    file_name=f"{ticker}_grafico.jpg",
                                    mime="image/jpeg",
                                    key=f"jpg_{ticker}"
                                )
                        else:
                            st.warning(f"❌ Imagem do gráfico não disponível para {ticker}")
    ...
    for i, ticker in enumerate(tickers):
        ...
        try:
            ...
            with st.container():
                st.subheader(f"{ticker} - {nome}")
                # Botão: salvar como favorito
                if st.button(f"⭐ Adicionar {ticker} aos favoritos", key=f"fav_exec_{ticker}"):
                    try:
                        uid = st.session_state.user["localId"]
                        fav_ref = db.reference(f"favoritos/{uid}/{ticker}")
                        fav_ref.set({
